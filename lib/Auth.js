 ...
 */
export function upsert(req, { code }) {
  return new Promise(async (resolve, reject) => {
    try {
      Debug(code)
      let { data } = await axios.post(`${baseUrl()}/api/auth/login`, { code }, {
        headers: req && req.headers.cookie ? { cookie: req.headers.cookie } : undefined,
      })
      if (data) return resolve(data)
      else return reject(null)
    } catch (error) {
      Debug(error)
      // console.log('Auth: upsert error', error)
      return reject(error)
    }
  })
}
...
export function profile(req) {
  return new Promise(async (resolve, reject) => {
    try {
      let { data } = await axios.get(`${baseUrl()}/api/auth/me`, {
...
export function keys(req) {
  return new Promise(async (resolve, reject) => {
    try {
      let { data } = await axios.get(`${baseUrl()}/api/auth/keys`, {
        headers: req ? { cookie: req.headers.cookie } : undefined,
      })
      if (data) return resolve(data)
      else return reject(null)
...
export function deleteKey(id, options) {
  return new Promise(async (resolve, reject) => {
    try {
      let req = options ? options.req || null : null
      let { data } = await axios.delete(`${baseUrl()}/api/auth/keys/${id}`, {
        headers: req ? { cookie: req.headers.cookie } : undefined,
      })
      if (data) return resolve(data)
      else return reject(null)
...
export function fetchSheet(id, options) {
  return new Promise(async (resolve, reject) => {
    try {
      let req = options ? options.req || null : null
      let { data } = await axios.get(`${baseUrl()}/api/auth/fetch-sheet/${id}`, {
        headers: req ? { cookie: req.headers.cookie } : undefined,
      })
      if (data) return resolve(data)
      else return reject(null)
