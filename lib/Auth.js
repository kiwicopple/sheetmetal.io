...
export function upsert(req, { code }) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let { data } = await axios.post(`${base}/api/auth/login`, { code }, {
...
export function deleteKey(id, options) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let base = req ? baseUrl(req) : ''
      let { data } = await axios.delete(`${base}/api/auth/keys/${id}`, {
        headers: req ? { cookie: req.headers.cookie } : undefined,
      })
      if (data) return resolve(data)
      else return reject(null)
...
export function fetchSheet(id, options) {
  return new Promise(async (resolve, reject) => {
    try {
      let req = options ? options.req || null : null
      let base = req ? baseUrl(req) : ''
      let { data } = await axios.get(`${base}/api/auth/fetch-sheet/${id}`, {
        headers: req ? { cookie: req.headers.cookie } : undefined,
      })
      if (data) return resolve(data)
      else return reject(null)
